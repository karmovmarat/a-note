    ...
    return (
      <form className="Form">{this.props.fields.map(field => {
        ...
        if (!this.props.readonly) {
          return (
            <div className={this.props.choose ? "FormRowChoose" : "FormRow"} key={field.id}>
              <label className={this.props.choose ? "FormLabelChoose" : "FormLabel"} 
                     htmlFor={field.id}>{field.label}: </label>
              <FormInput {...field} 
                ref={field.id} 
                defaultValue={prefilled} 
                getNewNote={this.props.getNewNote}  />
            </div>
          );
        }
        if (!prefilled) {
