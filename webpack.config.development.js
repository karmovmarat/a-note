const path = require('path');
 // для девелопмент: const webpack = require('webpack');
 // для девелопмент: const ExtractTextPlugin = require('extract-text-webpack-plugin');
...
context: __dirname, //path.resolve(__dirname, './src/js'),
  ...
  module: {
    ...
    rules: [
      ...
      {
            test: /\.scss$/,
           // для девелопмент:   use: ExtractTextPlugin.extract( {
            use: ['style-loader','css-loader', {
                loader: 'postcss-loader',
                options: {
                    plugins: () => [require('autoprefixer')]
  ...
  devServer: {
   ...
   plugins: [
       ...
       // для девелопмент:   new ExtractTextPlugin('app.css'),
